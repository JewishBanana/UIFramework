			...
			id.abilityMap.forEach((k, v) -> {
				...
				for (Action action : v)
					if (action.isPassive()) {
						...
						actions += action.getLangName()+' ';
				lore.add(UIFUtils.convertString(actions+k.getType().getName()+' '+k.getType().getDescription()));
			});
		}
		if (tempMeta.getEnchants().size() > 0) {
			tempMeta.addItemFlags(ItemFlag.HIDE_ENCHANTS);
			if (!(enchanted && tempMeta.getEnchants().size() == 1 && tempMeta.hasEnchant(Enchantment.DURABILITY))) {
				lore.add(" ");
				tempMeta.getEnchants().forEach((k, v) -> {
					if (!enchanted || k != Enchantment.DURABILITY)
